...
- (UIImage *)getBlurredImage:(UIImage *)imageToBlur {
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {
        return [imageToBlur applyBlurWithRadius:10.0f tintColor:[UIColor colorWithRed:0.0f/255.0f green:0.0f/255.0f blue:0.0f/255.0f alpha:0.7f] saturationDeltaFactor:1.0 maskImage:nil];
    }
    return imageToBlur;
}
...
- (void)presentPopupViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    if (self.popupViewController == nil) {
        ...
        //이상한 버그 받아온 뷰 컨트롤러의 프레임을 강제로 320 504 로 바꾼다.
        if ((frame.size.width == 320.f && frame.size.height == 504) || (frame.size.width == 320.f && frame.size.height == 416)) {
            viewControllerToPresent.view.frame = CGRectMake(0, 0, 280, 300);
        }
...
- (void)pushPopupViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    if (self.popupViewController == nil) {
        // initial setup
        self.popupViewController = viewControllerToPresent;
        self.popupViewController.view.autoresizesSubviews = NO;
        ...
        //이상한 버그 받아온 뷰 컨트롤러의 프레임을 강제로 320 504 로 바꾼다.
        if ((frame.size.width == 320.f && frame.size.height == 504) || (frame.size.width == 320.f && frame.size.height == 416)) {
            viewControllerToPresent.view.frame = CGRectMake(320.0f, 0, 280, 300);
        }
        ...
        if (self.useBlurForPopup) {
            ...
            if (UIDeviceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation)) {
                ...
                fadeView.frame = CGRectMake(320, 0, [UIScreen mainScreen].bounds.size.height, [UIScreen mainScreen].bounds.size.width);
            }
            fadeView.backgroundColor = [UIColor blackColor];
            fadeView.alpha = 0.0f;
            [self.view addSubview:fadeView];
        ...
        // setup
        if (flag) { // animate
            CGRect initialFrame = CGRectMake(320.0f, [UIScreen mainScreen].bounds.size.height + viewControllerToPresent.view.frame.size.height/2, finalFrame.size.width, finalFrame.size.height);
            NSLog(@"%@",NSStringFromCGRect(initialFrame));
            
            //viewControllerToPresent.view.frame = initialFrame;
            viewControllerToPresent.view.frame = CGRectMake(320.0f , 0.0f, 320.0f, 330.0f);
//            viewControllerToPresent.view.alpha = 0.0f;
            ...
            [UIView animateWithDuration:ANIMATION_TIME
                             ...
                             animations:^{
//                                 viewControllerToPresent.view.alpha = 1.0f;
                                 viewControllerToPresent.view.frame =  CGRectMake(0.0f, 0.0f, finalFrame.size.width, finalFrame.size.height);;
                                 blurView.alpha = self.useBlurForPopup ? 1.0f : 0.3f;
                             } completion:^(BOOL finished) {
                                 [self.popupViewController viewDidAppear:YES];
                                 [completion invoke];
                             }];
